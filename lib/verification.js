...
export function verifyProof(goal, proof) {
  ...
  if (goal.type === "single") {
    ...
    const scheduled = Array.isArray(goal.target?.scheduledTimes)
      ? goal.target.scheduledTimes
      : null;
    if (!scheduled || scheduled.length === 0) {
      return { ok: false, reason: "goal_missing_schedule", distanceMeters };
    }
    const windowMs =
      typeof goal.target?.windowMinutes === "number" && goal.target.windowMinutes > 0
        ? goal.target.windowMinutes * 60 * 1000
    ...
    // Find the *closest* unclaimed scheduled instant that the proof falls
    // inside the window of. Already-credited slots can't be re-claimed —
    // that prevents one selfie from being recycled across multiple sessions
    // (e.g. "I went Monday morning, here's the same photo for Tuesday").
    const credited = new Set(
      Array.isArray(goal.progress?.creditedTimes)
        ? goal.progress.creditedTimes.map((t) =>
            t instanceof Date ? t.toISOString() : new Date(t).toISOString()
          )
        : []
    );

    let bestSlotIso = null;
    let bestDelta = Infinity;
    for (const t of scheduled) {
      const slotMs = new Date(t).getTime();
      if (Number.isNaN(slotMs)) continue;
      const slotIso = new Date(slotMs).toISOString();
      if (credited.has(slotIso)) continue;
      const delta = Math.abs(captured - slotMs);
    ...
    return {
      ...
      reason: "ok",
      distanceMeters,
      matchedScheduledAt: bestSlotIso,
    };
  } else {
    // Unknown goal type: refuse rather than silently verify on geofence alone.
    // The API currently rejects anything other than "single" at create time,
    // so reaching here means the DB has legacy or malformed data.
    return { ok: false, reason: "unknown_goal_type", distanceMeters };
  }
