...
The [TokenEscrow amendment](/resources/known-amendments#tokenescrow) extends escrow functionality to fungible tokens, which means [Trust Line Tokens](/docs/concepts/tokens/fungible-tokens/trust-line-tokens) and [Multi-Purpose Tokens (MPTs)](/docs/concepts/tokens/fungible-tokens/multi-purpose-tokens#transferability-controls) can be held in escrow.
...
1. The sender creates an escrow using the `EscrowCreate` transaction. This transaction defines:
  ...
  - The conditions to release the funds.
    ...
    - For **token escrows** similar conditions apply, but an expiration time is *mandatory*.
...
[![State diagram showing escrows going from Held → Ready/Conditionally Ready → Expired](/assets/escrow-states.33821fcd0d2ebe57016c5053ea22a65b1f4962d0a3969ba36960095f7e8a24d6.ac57e6ef.png)](/assets/escrow-states.33821fcd0d2ebe57016c5053ea22a65b1f4962d0a3969ba36960095f7e8a24d6.ac57e6ef.png)
...
When using crypto-conditions, the EscrowFinish transaction must pay a [higher transaction cost](/docs/concepts/transactions/transaction-cost#special-transaction-costs) because of the higher processing load involved in verifying the crypto-condition fulfillment.

The additional transaction cost required is proportional to the size of the fulfillment. If the transaction is [multi-signed](/docs/concepts/accounts/multi-signing), the cost of multi-signing is added to the cost of the fulfillment.

Currently, an EscrowFinish with a fulfillment requires a minimum transaction cost of **330 [drops of XRP](/docs/references/protocol/data-types/basic-data-types#specifying-currency-amounts)** plus 10 drops per 16 bytes in the size of the fulfillment.

The above formula is based on the assumption that the reference cost of a transaction is 10 drops of XRP.

If [Fee Voting](/docs/concepts/consensus-protocol/fee-voting) changes the `reference_fee` value, the formula scales based on the new reference cost. The generalized formula for an EscrowFinish transaction with a fulfillment is as follows:
...
- **Escrow Tutorials:**
  - [Send a Timed Escrow](/docs/tutorials/payments/send-a-timed-escrow)
  - [Send a conditional Escrow](/docs/tutorials/payments/send-a-conditional-escrow)
  - [Look Up Escrows](/docs/tutorials/payments/look-up-escrows)
  - [Cancel an Expired Escrow](/docs/tutorials/payments/cancel-an-expired-escrow)
- [Transaction Reference](/docs/references/protocol/transactions)
  ...
  - [EscrowCancel transaction](/docs/references/protocol/transactions/types/escrowcancel)
- [Ledger Reference](/docs/references/protocol/ledger-data)
